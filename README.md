...
## Tools Usage
```python
python 锐捷EG易网关newcli.php执行交换机命令接口暴露.py  -h
usage: 锐捷EG易网关newcli.php执行交换机命令接口暴露.py [-h] (-u URL | -f FILE) [--random-agent RANDOM_AGENT] [--time-out TIME_OUT]
                                       [-d DELAY] [-t THREAD] [--proxy PROXY]

Ruijie EG Easy Gateway newcli.php Execution Switch Command Interface Exposed
